...
export default function UserNavbar() {
    ...
    return (
      <header className={`flex w-full h-[60px] sticky top-0 items-center py-6 px-10 lg:px-20 justify-between z-50 transition ease-in-out delay-50 duration-300 bg-white shadow`}>
        ...
        <div className={`hidden md:flex items-center [&>*]:text-base space-x-4 lg:space-x-10`} id="navbar-sticky">
          <Link href="/dashboard" className="mt-1 transition ease-in-out delay-50 duration-200 border-b-2 border-transparent hover:border-black">
            Dashboard
          </Link>
          ...
          <Link href="/" className="transition ease-in-out delay-50 duration-200">
            <FaCircleUser className="text-2xl text-blue-400 hover:text-blue-500 hover:drop-shadow-xl transition ease-in-out delay-30 duration-200"/>
          </Link>
        </div>
      </header>
    )
